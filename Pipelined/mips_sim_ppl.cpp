...
struct pipeline{    //registers between pipeline stages (not all ended up being used)
    int rs;
    int rt;
    int rd;
    int wr;
...
string trim(string in){ //removes 0s infront of numbers
    int l=in.length();
    bool found=0;
    string out="";
...
unsigned int directToInt(string hex) {  //translates HEX to int without considering 2s complement
    unsigned int sum=0;
...
string deformater(string in, string inop){  //ADD BACK SPACES TO COMMANDS FOR OUTPUT FILE
    string out="";
...
long msbPC(){   //=====deprecated
	long msb=PC & (~(268435455));
	//cout<<endl<<msb<<endl;
	return msb;
}

void earlyBranch(){ //completes branch command from the second stage of pipeline
...
void IFflush(bool fl){  //fluses the first stage of pipeline
    if(fl){
        IFID.ALUsrcA=0;
        IFID.ALUsrcB=0;
        IFID.immed=0;
...
void bubbles(){//nullifies the second stage of pipeline by disabling all its control signals
    IFID.memwrite=0;
    IFID.regwrite=0;
    IFID.branch=0;
...
void hazardDetectionAndForwarding(){    //detects hazards and sends forwarding signals to neutralize them
    IDEX.ALUsrcA=0;
    IDEX.ALUsrcB=0;
    hazard=0;
    forwarding=0;
...
void brnch(){   //implements branch commands
    //target=2;
...
void update(){//moves data from each pipeline stage to the next
